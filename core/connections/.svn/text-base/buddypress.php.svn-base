...
class WPC_BuddyPress {
        ...
        <?php if ( !empty($group_object->fields) && is_array($group_object->fields) ): ?>
          <?php foreach($group_object->fields as $field_key => $field_object): ?>
            <tr class="wp_crm_user_entry_row">
              <th>
                <?php ob_start();?>
                <label><?php _e($field_object->name, 'buddypress'); ?></label>
  ...
  /**
   * Renders Budypress profile field
   * 
    ...
    switch ($type) {
      ...
      case 'multiselectbox':
        ...
        <?php if ( !bp_get_the_profile_field_is_required() ) : ?>
          <a class="bp-clear-value" href="javascript:bp_clear_profile_field( '<?php echo $input_type; ?>' );"><?php _e( 'Clear', 'buddypress' ); ?></a>
        <?php endif; ?>
        </div>
        <?php
        break;
      case 'radio':
        ...
        ?>
        <?php if ( !bp_get_the_profile_field_is_required() ) : ?>
          <a class="bp-clear-value" href="javascript:bp_clear_profile_field( '<?php bp_the_profile_field_input_name(); ?>' );"><?php _e( 'Clear', 'buddypress' ); ?></a>
        <?php endif; ?>
        </div>
        <?php
        break;
  ...
  function add_crm_button() {
    ...
    <div id="view_crm_profile" class="btn">
      <a target="_blank" href="<?php echo admin_url("admin.php?page=wp_crm_add_new&user_id={$bp->displayed_user->id}"); ?>"><?php _e('View CRM Profile'); ?></a>
    </div>
    <?php
    }
  }
  ...
  function bp_save_profile_data($data) {
    ...
    //** There are errors */
    if ( $errors ) {
      WP_CRM_F::add_message(__('Please make sure you fill in all required Buddypress fields in this profile field group before saving.', 'crm'), 'bad');
    //** No errors */
    } else {
      ...
      if ( $errors ) {
        WP_CRM_F::add_message(__( 'There was a problem updating some of Budypress profile information, please try again.', 'crm' ), 'bad');
      }
    }
  }
